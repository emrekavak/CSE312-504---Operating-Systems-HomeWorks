...
#include "memoryBase.h" // neden memory base include edilmis ?
...
//
typedef struct ConditionCodes {  // bunlar ne amacla kullanılacak ?
	uint8_t		cy:1;
	uint8_t		pad:1;
	uint8_t		p:1;
	uint8_t		pad2:1;
...
typedef struct State8080 { // registers ?
	uint8_t		a;
	uint8_t		b;
	uint8_t		c;
	uint8_t		d;
	...
	uint16_t	pc;
//	uint8_t		*memory;
	struct ConditionCodes		cc; // ne icin ?
	uint8_t		int_enable;
...
class CPU8080 {
	friend class GTUOS; // GTUOS BURAYA ERİSEBİLECEK
public:
        CPU8080(MemoryBase * mem);
		~CPU8080();
        unsigned Emulate8080p(int debug = 0); // BU NE İCİN?
        bool isHalted() const;
        bool isSystemCall() const;
		void ReadFileIntoMemoryAt(const char* filename, uint32_t offset);
private:
        State8080 * state; // registerlar icin
        MemoryBase * memory; 
